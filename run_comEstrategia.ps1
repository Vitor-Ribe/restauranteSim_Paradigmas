# Compila o arquivo comEstrategia.kt
Write-Host "Compilando comEstrategia.kt..."
kotlinc comEstrategia.kt -include-runtime -d comEstrategia.jar

Write-Host "`n--- Executando comEstrategia.kt 10 vezes ---"
...
for ($i = 1; $i -le $executions; $i++) {
    Write-Host "`n--- Execução $i de $executions (comEstrategia) ---"
    $output = java -jar comEstrategia.jar | Out-String # Captura a saída do console
    Write-Host $output
    ...
    if ($totalProntos -eq 100) {
        ...
        Write-Host "  ❌ ERRO GRAVE: Pedidos duplicados detectados! Total prontos: $totalProntos (esperado: 100)."
        $currentResult.Errors += "Pedidos duplicados"
    } else { # $totalProntos -lt 100
        Write-Host "  ❌ ERRO GRAVE: Pedidos perdidos detectados! Total prontos: $totalProntos (esperado: 100)."
...
Write-Host "`n--- Resumo Final de Todas as ${executions} Execuções (comEstrategia) ---"
